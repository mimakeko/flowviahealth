...
- Data Stewardship action banners must use fixed safe result/error text only. Successful actions must not surface `NEXT_REDIRECT`; failures must not surface stack traces, Prisma errors, secrets, provider payloads, raw SMS bodies, or full phone numbers.
...
- Run `pnpm data:action-ux-smoke` and `pnpm data:demo-smoke` after changes to Data Stewardship reset, archive, demo scenario seeding, action-result banners, health flags, or audit filters.
