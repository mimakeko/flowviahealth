...
async function therapistReferralAction(formData: FormData) {
  ...
  const actionConfig = THERAPIST_ACTIONS.find((item) => item.value === action);
...
export default async function MyWorkPage({
  ...
  const selectedTherapistId = session.role === "admin" && params?.therapistId && therapists.some((therapist) => therapist.id === params.therapistId)
    ? params.therapistId
    : therapists[0]?.id;
    ...
    <div>
        ...
        {session.role === "admin" ? (
          ...
          <div className="mt-8 rounded-lg border border-line bg-white p-5">
            <p className="text-sm font-semibold text-ink">{therapists[0]?.name || "Therapist record not linked"}</p>
        ...
        <div className="mt-8 grid gap-5">
          {referrals.map((referral) => (
            <article key={referral.id} className="rounded-lg border border-line bg-white p-5">
                  ...
                  {referral.visits.map((visit) => (
                    <div key={visit.id} className="rounded-lg border border-line p-4">
                      ...
                      <form action={therapistVisitAction} className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                        ...
                        <label className="text-sm font-semibold text-ink">Visit status<select className="field" name="status" defaultValue={visit.status}>{THERAPIST_VISIT_STATUSES.map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}</select></label>
              ...
              <form action={therapistReferralAction} className="mt-5 grid gap-4 border-t border-line pt-5 md:grid-cols-[1fr_1fr_auto]">
                ...
                <label className="text-sm font-semibold text-ink">Action<select className="field" name="action" defaultValue="contacted">{THERAPIST_ACTIONS.map((action) => <option key={action.value} value={action.value}>{action.label}</option>)}</select></label>
          ...
          {selectedTherapistId && referrals.length === 0 ? (
