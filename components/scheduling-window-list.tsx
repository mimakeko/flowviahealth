...
export function SchedulingWindowList({ windows }: { windows: readonly SuggestedSchedulingWindow[] }) {
  return (
    <div className="rounded-lg border border-line bg-slate-50 p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-sm font-semibold text-ink">Suggested windows</h3>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        {windows.map((window) => (
          <div key={window.localInputValue} className="rounded-md border border-line bg-white px-3 py-2 text-sm">
            ...
            <p className="mt-1 text-xs text-slate-500">Review and enter manually in the visit form.</p>
          </div>
        ))}
